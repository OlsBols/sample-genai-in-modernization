...
// The comprehensive ARR validation prompt
const ARR_VALIDATION_PROMPT = `I need you to analyze an AWS Pricing Calculator CSV file for a MAP migration opportunity and provide a comprehensive ARR validation and gap analysis.

**Context:**
- This is a MAP (Migration Acceleration Program) opportunity
- Partners typically underestimate non-compute infrastructure by 10x
- We need to identify missing services across 6 critical categories

**Analysis Required:**

1. **ARR VALIDATION**
- Extract and calculate total annual ARR from the CSV
- Break down ARR by service category:
...
- Compare against MAP benchmark (56% compute, 44% non-compute)

2. **GAP ANALYSIS - 6 CRITICAL CATEGORIES**

**Category 1: Backup & Recovery**
- Check for: AWS Backup, database backup storage (Aurora/RDS), EBS snapshots, S3 Glacier, cross-region backup replication
- Common gap: Partners estimate database compute but forget backup storage
- Expected: 2-3% of total infrastructure ARR
...
- Common gap: Partners estimate S3 Standard but miss FSx, Storage Gateway, S3 Intelligent-Tiering
- Expected: 25-30% of total infrastructure ARR
...
- Common gap: Partners estimate single-region only, no DR orchestration
- Expected: 1-2% of total infrastructure ARR
...
- Common gap: Partners estimate load balancers but miss CDN, DNS, data transfer costs, IPv4 addressing
- Expected: 10-15% of total infrastructure ARR
- Questions: "Do you serve content globally? How much outbound traffic? How many public IPs? Even though CCOE/central team is managing networking, the increase in traffic wrt NAT Gateway, Transit Gateway Attachment should be included in ARR, isn't it?"
...
- Common gap: Partners assume monitoring is "free" or "included"
- Expected: 2-4% of total infrastructure ARR
...
- Common gap: Partners include only KMS, miss application-layer security and credential management
- Expected: 2-4% of total infrastructure ARR
...
**A. ARR VALIDATION SUMMARY**
\`\`\`
Total Annual ARR: $XXX,XXX
Breakdown:
- Compute: $XXX,XXX (XX%)
- Storage: $XXX,XXX (XX%)
- Database: $XXX,XXX (XX%)
...
MAP Benchmark: 56% / 44%
Assessment: [On target / Underestimated / Overestimated]
\`\`\`

**B. GAP ANALYSIS BY CATEGORY**

For each of the 6 categories, provide:
\`\`\`
Category: [Name]
Status: [Complete / Partial / Missing]
Current ARR: $XXX,XXX (XX% of total)
Expected ARR: $XXX,XXX (XX% of total)
Gap: $XXX,XXX
Services Found: [List]
Services Missing: [List with estimated ARR impact]
Priority: [High / Medium / Low]
Questions to Ask Partner: [Specific questions]
\`\`\`

**C. MISSING SERVICES SUMMARY**

Create a prioritized table:
| Priority | Service | Category | Estimated ARR | Why It's Missing | Questions to Ask |
|----------|---------|----------|---------------|------------------|------------------|
| HIGH     | ...     | ...      | $XX,XXX       | ...              | ...              |
...
**E. ESTIMATED ADDITIONAL ARR POTENTIAL**
\`\`\`
Conservative Estimate: $XXX,XXX - $XXX,XXX/year
Aggressive Estimate: $XXX,XXX - $XXX,XXX/year
\`\`\`
...
- ❌ Security services <1% of total ARR
- etc.

**G. NEXT STEPS**

Provide 5-7 specific action items for the partner.

4. **KEY VALIDATION CHECKS**

Flag these red flags:
- ❌ No backup services (AWS Backup, backup storage)
...
function HiddenARRDiscovery() {
  ...
  const [analysis, setAnalysis] = useState(hiddenARRData?.analysis || null);
  const [recordsProcessed, setRecordsProcessed] = useState(0);
  const [activeTabId, setActiveTabId] = useState('upload');
  
  // Prompt customization
  const [customPrompt, setCustomPrompt] = useState(ARR_VALIDATION_PROMPT);
  const [useCustomPrompt, setUseCustomPrompt] = useState(false);
  
  // Load existing data from context on mount
  useEffect(() => {
    if (hiddenARRData?.analysis) {
      setAnalysis(hiddenARRData.analysis);
  ...
  useEffect(() => {
    const saved = localStorage.getItem('map_custom_prompt_hidden_arr');
    if (saved) {
  ...
  const saveCustomPrompt = () => {
    localStorage.setItem('map_custom_prompt_hidden_arr', customPrompt);
  ...
  const resetPrompt = () => {
    if (confirm('Reset prompt to default? This cannot be undone.')) {
      setCustomPrompt(ARR_VALIDATION_PROMPT);
      setUseCustomPrompt(false);
      localStorage.removeItem('map_custom_prompt_hidden_arr');
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      formData.append('custom_prompt', useCustomPrompt ? customPrompt : ARR_VALIDATION_PROMPT);

      const response = await fetch(getApiUrl('/map/hidden-arr/analyze'), {
        method: 'POST',
        body: formData
      });
      ...
      // Save to context
      setHiddenARRData({ analysis: result.analysis });
      // Switch to results tab
      setActiveTabId('results');
    } catch (err) {
      setError(err.message || 'Failed to analyze ARR');
    } finally {
      setLoading(false);
    }
  };
    ...
    <SpaceBetween size="l">
      <Container
        header={
          <Header
            variant="h1"
            description="Identify Missing Services & ARR Gaps in AWS Calculator Estimates"
            actions={
        ...
        <SpaceBetween size="m">
          <Alert type="info">
            <SpaceBetween size="xs">
              <Box variant="strong">Uncover Hidden Revenue Opportunities</Box>
              <Box>
                Upload your AWS Pricing Calculator CSV to discover missing services across 6 critical 
                categories: Backup & Recovery, Storage, DR/HA, Network, Observability, and Security. 
                Partners typically underestimate non-compute infrastructure by 10x.
              </Box>
              <ColumnLayout columns={3} variant="text-grid">
                <div>
                  <Box variant="awsui-key-label">MAP Benchmark</Box>
                  <Box>56% Compute / 44% Non-Compute</Box>
                </div>
                <div>
                  <Box variant="awsui-key-label">Expected Gap</Box>
                  <Box>$100K - $500K+ Additional ARR</Box>
                </div>
                <div>
            ...
            tabs={[
                ...
                content: (
                  <SpaceBetween size="l">
                    <FormField
                      ...
                      description="Upload your AWS Calculator export file for comprehensive ARR validation"
                    >
                      <FileUpload
                    ...
                    >
                      <SpaceBetween size="m">
                        <Box variant="h4">6 Critical Categories Analyzed:</Box>
                      ...
                      <Box textAlign="center" padding="l">
                        <SpaceBetween size="m" alignItems="center">
                          ...
                          <Box variant="p" color="text-body-secondary">
                            Analyzing ARR and identifying gaps... This may take 2-3 minutes.
                          </Box>
                        </SpaceBetween>
                      </Box>
                    )}
              ...
              },
              {
                ...
                content: (
                  <SpaceBetween size="l">
                    <Alert type="info">
                      The default ARR validation prompt is shown below. Toggle "Use Custom Prompt" to modify it for your specific needs.
                    ...
                    >
                      <SpaceBetween size="s">
                        <Box variant="p">
                          <strong>Tips for customizing the ARR validation prompt:</strong>
                        </Box>
                        <ul>
                          ...
                          <li>Modify the questions to ask partners based on your discovery process</li>
                          <li>Change the priority thresholds for red flags</li>
                          <li>Add industry-specific compliance requirements</li>
                          <li>Customize the output format to match your reporting needs</li>
                          <li>Include specific AWS programs or incentives relevant to your region</li>
                        </ul>
                      </SpaceBetween>
                    </ExpandableSection>
                  </SpaceBetween>
              ...
              },
              {
                ...
                content: analysis && (
                  <SpaceBetween size="m">
                    <Alert type="success">
                      <SpaceBetween size="xs">
                        <Box variant="strong">ARR Gap Analysis Complete</Box>
                        <Box>
                          Review the comprehensive analysis below to identify missing services and 
                          additional ARR opportunities. Use the SMART action items to engage with your partner.
                        </Box>
                      </SpaceBetween>
                    </Alert>

                    <ExpandableSection
                      headerText="Hidden ARR Discovery Results"
                      variant="container"
                      defaultExpanded
                    >
                      <SpaceBetween size="m">
                        ...
                        <Button
                          onClick={() => downloadMarkdown(analysis, 'hidden_arr_discovery_analysis.md')}
                          iconName="download"
                        >
                    ...
                    <Alert type="info">
                      <SpaceBetween size="xs">
                        ...
                        <Box>
                          1. Review the gap analysis and missing services<br/>
                          2. Schedule a discovery call with your partner<br/>
                          3. Ask the specific questions provided for each category<br/>
                          4. Update the AWS Calculator with recommended services<br/>
                          5. Re-run this analysis to validate the updated ARR
                        </Box>
                      </SpaceBetween>
                    </Alert>
                  </SpaceBetween>
...
export default HiddenARRDiscovery;
