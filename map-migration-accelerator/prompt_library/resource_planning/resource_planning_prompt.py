def get_resource_planning_prompt(migration_strategy, wave_planning_data, resource_details):
    resource_prompt = f"""
        ...
        **FORMATTING RULES — STRICTLY FOLLOW:**
        - Use bullet points (- item), NOT dense paragraphs
        - Executive Summary: exactly 5 concise bullet points, one line each
        - Team Structure sections: 3-5 bullet points per model, one line each
        - Comparative Analysis: 5-7 bullet points covering shared elements and key differences
        - Final Recommended Structure: 5 bullet points max
        - Justification sections: 3-5 bullet points each, one line per point
        - Tables should have only the rows needed (no empty placeholder rows)
        - Do NOT write multi-sentence paragraphs — every point must be a separate bullet
